 ...
 *that each invocation will free the previous aiScene. If you really want to stick to aiScene, 
 ...
 This implementation uses ONE importer for two object scenes (aiScene).The importer object is declared as global in order
// to persist during whole application execution. Since we convert one aiScene object to MyMesh array immediately after the Import.Read file,
// we can use the same importer for reading another object scene (aiScene). The RenderScene renders two MyMeshes vectors.
 *
...
bool LoadGLTexturesTUs(const aiScene*& scene, Texture &texObjectArray, unordered_map<std::string, GLuint>& textureIdMap)  // Create OGL textures objects and maps them to texture units.  
{
	...
	printf("Total number of texture maps= %d\n", numTextures);
	if(numTextures) {
		...
		{
			filename = (*itr).first;  // get filename
			texObjectArray.texture2D_Loader(filename.c_str());  //it already performs glBindTexture(GL_TEXTURE_2D, textureIds[i])
...
void aiRecursiveNdTraverse(const aiScene*& sc, const aiNode* nd, vector<struct MyMesh>& myMeshes, unordered_map<std::string, GLuint>& textureIdMap, gmu &mu)
{
	GLuint buffer;
	...
	//MODEL will contain the current node Transform
	mu.multMatrix(gmu::MODEL, transform);
	
	// create all MyMeshes assigned to this node
	for (unsigned int n = 0; n < nd->mNumMeshes; ++n) {
		...
		struct MyMesh aMesh;  //Mymesh will store the Assimp mesh

		memcpy(aMesh.transform, mu.get(gmu::MODEL), sizeof(float) * 16);  //MyMesh transform stores the node Transform
	...
	// Create the MyMeshes for all children
	for (unsigned int n = 0; n < nd->mNumChildren; ++n) {
		aiRecursiveNdTraverse(sc, nd->mChildren[n], myMeshes, textureIdMap, mu);
	}

	mu.popMatrix(gmu::MODEL);
}

//vector<struct MyMesh> createMyMeshFromAssimp(const aiScene*& sc, GLuint*& textureIds) {
void createMyMeshFromAssimp(const aiScene*& sc, Renderer* render) {

	gmu mu;  // math utility class to handle MODEL, VIEW and PROJECTION stacks
	...
	printf("Total number of meshes in the scene = %d\n", sc->mNumMeshes);
	LoadGLTexturesTUs(sc, render->TexObjArray, textureIdMap); //it creates the unordered map which maps image filenames to texture units TU

	mu.loadIdentity(gmu::MODEL);
	//traverse the Assimp tree
	aiRecursiveNdTraverse(sc, sc->mRootNode, render->myMeshes, textureIdMap, mu);
